# Sirius-DB / MatrixOne Integration Design

## GPU-Accelerated Analytics via DuckDB TAE Scanner Extension

---

## 1. Executive Summary

This document describes the architecture for integrating **Sirius-DB** (a GPU-native
SQL execution engine built as a DuckDB extension) with **MatrixOne** (a cloud-native
HTAP database). The goal is to offload analytical workloads (TPC-H, ad-hoc OLAP) to
GPU hardware while MatrixOne continues to serve as the storage and transaction engine.

The integration is based on a **DuckDB scanner extension** that reads MatrixOne's TAE
(Table Analytics Engine) object files directly from the local filesystem (Phase 1) or
S3 (Phase 2). This avoids the ETL overhead of Parquet export and provides real-time
...
- No modifications to MatrixOne codebase required
...
│                        Sirius-DB                             │
│  ┌─────────┐  ┌──────────┐  ┌───────────┐  ┌────────────┐  │
│  │ Parser  │→│ Planner  │→│ GPU Exec  │→│ Result     │  │
│  │         │  │(DuckDB)  │  │ (CUDA)    │  │ Collector  │  │
│  └─────────┘  └────┬─────┘  └───────────┘  └────────────┘  │
│                     │ Scan Request                           │
│                     ▼                                        │
│  ┌──────────────────────────────────────────────────────┐   │
...
duckdb -cmd "LOAD 'tae_scanner.so'; LOAD 'sirius.so';"
...
to most sophisticated. All paths use `tae_scan()` as the data access layer and
...
sirius-sidecar --port 50051 \
    --extensions "sirius.so,tae_scanner.so" \
    --data-dir /mo-data/shared/
```
